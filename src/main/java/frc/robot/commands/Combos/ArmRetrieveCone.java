  ...
  public ArmRetrieveCone(ElevatorSubsystem elevator, ShoulderSubsystem shoulder, WristSubsystem wrist) {
    ...
    addCommands(
      new ParallelCommandGroup(
        new ElevatorGoToPosition(elevator, 119.41).withTimeout(2),
        new GotoPosShoulder(shoulder, Constants.Arm.Shoulder.targetPosition5).withTimeout(1),
        new GoToPositionWrist(wrist, 0.552).withTimeout(1)
      ).withTimeout(1.5)
    );
  }
}
